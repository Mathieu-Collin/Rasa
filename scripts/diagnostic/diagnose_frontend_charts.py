...
def test_rasa_response_format():
    ...
    try:
        response = requests.post(
            "http://localhost:6005/webhooks/rest/webhook",
...
def generate_frontend_recommendations(data: list):
    ...
    has_custom_field = any(isinstance(msg, dict) and "custom" in msg for msg in data)

    has_chart_type = any(isinstance(msg, dict) and msg.get("custom", {}).get("type") == "chart" for msg in data)
